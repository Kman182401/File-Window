...
class IBKRHealthMonitor:
    ...
    def check_port_accessibility(self) -> bool:
        ...
        # DISABLED: Port checks create socket storms
        # Health monitor should be completely passive in single socket mode
    ...
    def perform_health_check(self) -> HealthMetrics:
        ...
        if metrics.port_accessible:
            # DISABLED: Health monitor IB connections violate single socket pattern
            # The health monitor should only check port availability, not create IB connections
            if False:  # Disabled to maintain single socket pattern
                ...
                try:
                    ...
                    ib.connect(self.host, self.port, clientId=int(os.getenv('IBKR_HEALTH_CLIENT_ID','9102')),
                              timeout=10)
                ...
                # Health monitor connections disabled - just check port availability
                if metrics.port_accessible:
                    metrics.status = ConnectionHealth.HEALTHY
                    metrics.last_heartbeat = datetime.now()
                    self.last_successful_check = datetime.now()
    ...
    def start_monitoring(self):
        ...
        # SINGLE SOCKET MODE: Do not start monitor threads when disabled
        if os.getenv("DISABLE_HEALTH_MONITOR") == "1":
            logger.info("Health monitor disabled (single-socket mode); skipping thread start")
            return
    ...
    def diagnose_connection_issues(self) -> List[str]:
        ...
        if not metrics.docker_container_healthy:
            ...
            recommendations.append("Run: cd /home/karson/ibkr && docker compose restart")
