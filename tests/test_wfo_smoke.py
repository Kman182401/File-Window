...
def _synthetic_bars(days: int = 40, freq: str = "15T") -> pd.DataFrame:
    start = datetime(2024, 1, 1)
    periods = int((timedelta(days=days).total_seconds() // 60) / 15)
    index = pd.date_range(start=start, periods=periods, freq=freq, tz="America/New_York")
